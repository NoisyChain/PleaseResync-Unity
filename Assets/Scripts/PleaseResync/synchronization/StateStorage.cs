...
namespace PleaseResync
{
    internal class StateStorage
    {
        ...
        public void SaveFrame(int frame, byte[] stateBuffer, uint stateChecksum = 0)
        {
            ...
            //UnityEngine.Debug.Log($"Saved state at frame {frame} with checksum {stateChecksum}");
        }
        ...
        {
            UnityEngine.Debug.Log($"Checksum comparison: Frame:({frame} :: {_states[frame % _size].Checksum}), {checksum}");
