...
namespace PleaseResyncTest
{
    ...
    public class PleaseResyncTest_Peer2PeerSession
    {
        ...
        public void Test_RollbackSequence()
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                {
                    UnityEngine.Debug.Log(sessionEvents1.Dequeue().Desc());
                }

                while (sessionEvents2.Count > 0)
                {
                    UnityEngine.Debug.Log(sessionEvents2.Dequeue().Desc());
                }*/
                ...
                foreach (var action in sessionActions1)
                {
                    switch (action)
                    {
                        ...
                        case SessionLoadGameAction LGAction:
                            ...
                            sessionState1 = new TestHelpers.TestState(reader);//MessagePackSerializer.Deserialize<TestHelpers.TestState>(LGAction.Load());
                            break;
                        case SessionSaveGameAction SGAction:
                            MemoryStream writerStream = new MemoryStream();
                            BinaryWriter writer = new BinaryWriter(writerStream);
                            sessionState1.Serialize(writer);
                ...
                foreach (var action in sessionActions2)
                {
                    switch (action)
                    {
                        ...
                        case SessionLoadGameAction LGAction:
                            MemoryStream readerStream = new MemoryStream(LGAction.Load());
                            BinaryReader reader = new BinaryReader(readerStream);
                            sessionState2 = new TestHelpers.TestState(reader);//MessagePackSerializer.Deserialize<TestHelpers.TestState>(LGAction.Load());
                            break;
                        case SessionSaveGameAction SGAction:
                            MemoryStream writerStream = new MemoryStream();
                            BinaryWriter writer = new BinaryWriter(writerStream);
        ...
        /*public void Test_SyncInputAcrossDevices_StepByStep()
        {
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[1]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[1]).Inputs[1]); // local input
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions1[1]).Inputs[2]); // we got the remote inputs now :)
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions1[1]).Inputs[3]); // we got the remote inputs now :)
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[3]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[3]).Inputs[1]); // local input
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions1[3]).Inputs[2]); // we predict the input for the remote device to be the same as the previous frame
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions1[3]).Inputs[3]); // we predict the input for the remote device to be the same as the previous frame
            ...
            Assert.AreEqual(1, ((SessionAdvanceFrameAction)actions2[1]).Frame);
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions2[1]).Inputs[0]); // we got the remote inputs now :)
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions2[1]).Inputs[1]); // we got the remote inputs now :)
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions2[1]).Inputs[2]); // local input
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions2[1]).Inputs[3]); // local input
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions2[3]).Inputs[0]); // we predict the input for the remote device to be the same as the previous frame
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions2[3]).Inputs[1]); // we predict the input for the remote device to be the same as the previous frame
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions2[3]).Inputs[2]); // local input
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions2[3]).Inputs[3]); // local input
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[0]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[0]).Inputs[1]); // local input
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions1[0]).Inputs[2]); // predicted input
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions1[0]).Inputs[3]); // predicted input
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions2[0]).Inputs[0]); // predicted input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions2[0]).Inputs[1]); // predicted input
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions2[0]).Inputs[2]); // local input
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions2[0]).Inputs[3]); // local input
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[0]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[0]).Inputs[1]); // local input
            Assert.AreEqual((byte)3, ((SessionAdvanceFrameAction)actions1[0]).Inputs[2]); // predicted input (but wrong), should be rolled back on the next frame
            Assert.AreEqual((byte)4, ((SessionAdvanceFrameAction)actions1[0]).Inputs[3]); // predicted input (but wrong), should be rolled back on the next frame
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions2[0]).Inputs[0]); // predicted input (and right), should NOT be rolled back on the next frame
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions2[0]).Inputs[1]); // predicted input (and right), should NOT be rolled back on the next frame
            Assert.AreEqual((byte)5, ((SessionAdvanceFrameAction)actions2[0]).Inputs[2]); // local input
            Assert.AreEqual((byte)6, ((SessionAdvanceFrameAction)actions2[0]).Inputs[3]); // local input
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[1]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[1]).Inputs[1]); // local input
            Assert.AreEqual((byte)5, ((SessionAdvanceFrameAction)actions1[1]).Inputs[2]); // we got the remote inputs now :)
            Assert.AreEqual((byte)6, ((SessionAdvanceFrameAction)actions1[1]).Inputs[3]); // we got the remote inputs now :)
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[3]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[3]).Inputs[1]); // local input
            Assert.AreEqual((byte)5, ((SessionAdvanceFrameAction)actions1[3]).Inputs[2]); // predicted input
            Assert.AreEqual((byte)6, ((SessionAdvanceFrameAction)actions1[3]).Inputs[3]); // predicted input
            ...
            Assert.AreEqual((byte)1, ((SessionAdvanceFrameAction)actions1[0]).Inputs[0]); // local input
            Assert.AreEqual((byte)2, ((SessionAdvanceFrameAction)actions1[0]).Inputs[1]); // local input
            Assert.AreEqual((byte)5, ((SessionAdvanceFrameAction)actions1[0]).Inputs[2]); // predicted input
            Assert.AreEqual((byte)6, ((SessionAdvanceFrameAction)actions1[0]).Inputs[3]); // predicted input
            Assert.IsInstanceOfType(actions1[1], typeof(SessionSaveGameAction));
    ...
    public static class TestHelpers
    {
        ...
        public class TestState
        {
            ...
            public TestState(BinaryReader br)
            {
                Deserialize(br);
            }
        ...
        /*public static NativeArray<byte> ToBytes() {
        ...
        public static void FromBytes(NativeArray<byte> bytes) {
